...
if __name__=='__main__':
    ...
    # while True:
        ...
        _game_calender = 'https://baseball.yahoo.co.jp/npb/teams/6/schedule?month=' + tday.strftime('%Y-%m')
                        ...
                        _playerscore.append(
                            catcher 
                                if order_of_pitcher==0 or fullmask 
                                    ...
                                    get_catcher_name(
                                        int(input('次の投手 "'+_playerscore[1]+'" の捕手 (0:坂倉, 1:會澤, 2:石原, 3:その他)> '))
                                    )
                        )
            ...
            while nextrunner:
                ...
                if not fullmask:
                    catcher = get_catcher_name(
                                    int(input('  捕手 (0:坂倉, 1:會澤, 2:石原, 3:その他)> '))
                            )
                pitcher = input('  投手> ')
                ...
                with open(csvdir+'/stolen_stats'+tday.strftime('%y')+'c.csv',mode='a',encoding='shift-jis') as f:
        ...
        print()
        if int(input('広島にバッテリーエラーあり？(1:YES, 0:NO)> ')):
            nextbattery = True
            while nextbattery:
                if not fullmask:
                    catcher = get_catcher_name(
                ...
                with open(csvdir+'/pitch_stats'+tday.strftime('%y')+'c.csv',mode='a',encoding='shift-jis') as f:
                    # catcher,pitcher,day_of_game,wild_pitch,passed_ball,series
                    print(','.join(pdata))
                    print()
                    f.write(','.join(pdata)+'\n')

                nextbattery = int(input('  他にも広島にバッテリーエラーあり?(1:YES, 0:NO)> '))

        print('CSV UPDATED.')

        # CALENDER-HEATMAP UPDATE
        CALENDERHEATMAP = './assets/demo/calendar-heatmap-demo-c.js'
        _team = ''
        if team=='阪神':
            _team = 'Tigers'
        elif team=='中日':
        ...
        DATATABLESDEMO = './assets/demo/datatables-demo-c.js'
    ...
    # if False:
        ...
        replace_table('catcher_stats', 'catcher_stats'+TODAY.strftime('%y')+'c.csv')
        replace_table('stolen_stats', 'stolen_stats'+TODAY.strftime('%y')+'c.csv')
        ...
        sql_cat = " \
            SELECT \
            %s, \
            MAX(CASE tmp2.catcher WHEN '坂倉' THEN tmp2.wls ELSE NULL END) 坂倉, \
            MAX(CASE tmp2.catcher WHEN '會澤' THEN tmp2.wls ELSE NULL END) 會澤, \
            MAX(CASE tmp2.catcher WHEN '石原' THEN tmp2.wls ELSE NULL END) 石原 \
            FROM \
            ( \
            SELECT \
                catcher, \
        ...
        atag_tweet_html = '\t'*7 + '<a href="http://twitter.com/share?text='+TODAY.strftime('%Y')+'広島捕手別投手成績%0D%0A捕手, 勝-敗-S, 防御率 (イニング)%0D%0A'
        for catcher_stat in catcher_stats[1:]:
        ...
        with open('./assets/data/index.template-c.html',mode='r',encoding='utf-8') as f1:
            with open('./carp/index.html',mode='w',encoding='utf-8') as f2:
                for line in f1:
                    f2.write(
                        line
        ...
        with open('./assets/data/chart-pie-demo.template-c.js',mode='r',encoding='utf-8') as f1:
            with open('./assets/demo/chart-pie-demo-c.js',mode='w',encoding='utf-8') as f2:
                for line in f1:
                    f2.write(
                        line
                            .replace('{_pie_data}', str(pie_data))
                            .replace('[_catchers]', str(['坂倉', '會澤', '石原']))
                    )
