...
if __name__=='__main__':
    ...
    # while True:
        ...
        catcher = get_catcher_name(int(input('スタメン捕手 (0:梅野, 1:坂本, 2:その他)> ')))
        fullmask = int(input('フルマスク? (1:YES, 0:NO)> '))

        # get yahoo sports gameid
        _game_calender = 'https://baseball.yahoo.co.jp/npb/teams/5/schedule?month=' + tday.strftime('%Y-%m')
        ...
        for _k, news in enumerate(soup.findAll('section', class_='bb-modCommon02')):
            ...
            if _team is None:
                pass
            elif _team.text == '阪神タイガース':
                with open(csvdir+'/catcher_stats'+tday.strftime('%y')+'t.csv',mode='a',encoding='shift-jis') as f:
                        ...
                        _playerscore.append(
                            catcher 
                                if order_of_pitcher==0 or fullmask 
                                    ...
                                    get_catcher_name(
                                        int(input('次の投手 "'+_playerscore[1]+'" の捕手 (0:梅野, 1:坂本, 2:その他)> '))
                                    )
                        )
        ...
        if int(input('敵チームの盗塁企図あり? (1:YES, 0:NO)> ')):
            ...
            while nextrunner:
                ...
                with open(csvdir+'/stolen_stats'+tday.strftime('%y')+'t.csv',mode='a',encoding='shift-jis') as f:
        ...
        print()
        if int(input('阪神にバッテリーエラーあり？(1:YES, 0:NO)> ')):
            nextbattery = True
            while nextbattery:
                if not fullmask:
                    catcher = get_catcher_name(
                ...
                with open(csvdir+'/pitch_stats'+tday.strftime('%y')+'t.csv',mode='a',encoding='shift-jis') as f:
                    # catcher,pitcher,day_of_game,wild_pitch,passed_ball,series
                    print(','.join(pdata))
                    print()
                    f.write(','.join(pdata)+'\n')

                nextbattery = int(input('  他にも阪神にバッテリーエラーあり?(1:YES, 0:NO)> '))

        print('CSV UPDATED.')

        # CALENDER-HEATMAP UPDATE
        CALENDERHEATMAP = './assets/demo/calendar-heatmap-demo-t.js'
        _team = ''
        if team=='阪神':
            _team = 'Tigers'
        elif team=='中日':
        ...
        DATATABLESDEMO = './assets/demo/datatables-demo-t.js'
    ...
    # if False:
        ...
        replace_table('catcher_stats', 'catcher_stats'+TODAY.strftime('%y')+'t.csv')
        replace_table('stolen_stats', 'stolen_stats'+TODAY.strftime('%y')+'t.csv')
        ...
        sql_cat = " \
            SELECT \
            %s, \
            MAX(CASE tmp2.catcher WHEN '梅野' THEN tmp2.wls ELSE NULL END) 梅野, \
            MAX(CASE tmp2.catcher WHEN '坂本' THEN tmp2.wls ELSE NULL END) 坂本, \
            MAX(CASE tmp2.catcher WHEN '長坂' THEN tmp2.wls ELSE NULL END) 長坂 \
            FROM \
            ( \
            SELECT \
                catcher, \
        ...
        atag_tweet_html = '\t'*7 + '<a href="http://twitter.com/share?text='+TODAY.strftime('%Y')+'阪神捕手別投手成績%0D%0A捕手, 勝-敗-S, 防御率 (イニング)%0D%0A'
        for catcher_stat in catcher_stats[1:]:
        ...
        atag_tweet_html += '%23tigers%0D%0Akg1-thub.github.io/tigers/" target="_blank" data-toggle="tooltip" data-placement="bottom" title="X for sharing">'

        pie_data = {'Catcher':['梅野', '坂本', '長坂', 'その他'],
                ...
                for line in f1:
                    f2.write(
                        line
                            ...
                            .replace('[_catchers]', str(['梅野', '坂本', '長坂']))
                    )
